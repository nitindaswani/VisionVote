...
def send_otp_email(email, otp):
    msg = MIMEMultipart()
    msg['From'] = 'af0c7f001@smtp-brevo.com'
    msg['To'] = email
    msg['Subject'] = 'Vision Vote OTP Verification'
    msg.attach(MIMEText(f'Your OTP is {otp}', 'plain'))

    with smtplib.SMTP('smtp-relay.brevo.com', 587) as server:
        server.starttls()
        server.login('af0c7f001@smtp-brevo.com', 'gVtq1AB7RPnWwH8j')
        server.sendmail('af0c7f001@smtp-brevo.com', email, msg.as_string())
...
def send_voter_email(user):
    ...
    msg['From'] = 'af0c7f001@smtp-brevo.com'
    msg['To'] = user.email
    msg['Subject'] = 'Vision Vote Registration Successful'
    body = (
        f"Dear {user.full_name},\n\n"
        f"Your Vision Vote account has been created successfully.\n\n"
        f"Your Voter ID is:\n"
    ...
    with smtplib.SMTP('smtp-relay.brevo.com', 587) as server:
        server.starttls()
        server.login('af0c7f001@smtp-brevo.com', 'gVtq1AB7RPnWwH8j')
        server.sendmail('af0c7f001@smtp-brevo.com', user.email, msg.as_string())
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not otp_record:
                ...
                # Delete unverified user when OTP expires
                user.delete()
...
def face_register(request):
    ...
    return render(request, "accounts/face_register.html")
...
def save_face(request):
    start = time.time()
    print("START")
    if not request.session.get("blink_verified"):
        return JsonResponse({
            "success": False,
            "message": "Blink verification required"
        })
    ...
    FaceProfile.objects.update_or_create(
        user=user,
        defaults={"face_encoding": json.dumps(encodings[0].tolist())}
    )
    print("After DB save:", time.time() - start)

    threading.Thread(target=send_voter_email, args=(user,)).start()
    print("After email:", time.time() - start)

    request.session.pop("face_user_id", None)
    request.session.pop("blink_verified", None)
...
def verify_blink(request):
    ...
    request.session["blink_verified"] = True
...
def login_view(request):
    ...
    if request.method == "POST":
        ...
        if not user:
            error_message = "Invalid Voter ID or account disabled."
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not otp_record:
                ...
                request.session["face_login_user_id"] = user.id

                return redirect("accounts:face_login")
...
def verify_face(request):
    ...
    user_id = request.session.get("face_login_user_id")
    ...
    stored_encoding = np.array(
        json.loads(user.face_profile.face_encoding)
    )
    ...
    request.session["logged_in_user"] = user.id
    request.session.pop("vote_start_time", None)
    request.session.pop("vote_election_id", None)
    request.session.pop("face_login_user_id", None)
    request.session.pop("blink_verified", None)
